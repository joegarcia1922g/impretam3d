...
import { listContacts, upsertContact } from '../../../_shared/crm.js';
...
export async function onRequest(context) {
    ...
    if (request.method === 'GET') {
        const contacts = await listContacts(db, request);
        return jsonResponse({ ok: true, storage: 'd1', contacts });
    }

    if (request.method === 'POST') {
        const parsed = await readJsonBody(request);
        if (parsed.error) return jsonResponse({ ok: false, error: parsed.error }, parsed.error === 'payload_too_large' ? 413 : 400);
        const contact = await upsertContact(db, parsed.value || {});
        return jsonResponse({ ok: true, storage: 'd1', contact }, 201);
    }
