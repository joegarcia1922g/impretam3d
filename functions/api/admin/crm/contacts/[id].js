...
import { upsertContact } from '../../../../_shared/crm.js';
...
export async function onRequest(context) {
    ...
    if (!id) return jsonResponse({ ok: false, error: 'invalid_contact_id' }, 400);
    ...
    const contact = await upsertContact(db, parsed.value || {}, id);
    return jsonResponse({ ok: true, storage: 'd1', contact });
}
