...
import { listTasks, upsertTask } from '../../../_shared/crm.js';
...
export async function onRequest(context) {
    ...
    if (request.method === 'GET') {
        const tasks = await listTasks(db, request);
        return jsonResponse({ ok: true, storage: 'd1', tasks });
    }

    if (request.method === 'POST') {
        const parsed = await readJsonBody(request);
        if (parsed.error) return jsonResponse({ ok: false, error: parsed.error }, parsed.error === 'payload_too_large' ? 413 : 400);
        const task = await upsertTask(db, parsed.value || {});
        return jsonResponse({ ok: true, storage: 'd1', task }, 201);
    }
