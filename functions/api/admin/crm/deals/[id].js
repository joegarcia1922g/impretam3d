...
import { upsertDeal } from '../../../../_shared/crm.js';
...
export async function onRequest(context) {
    ...
    if (!id) return jsonResponse({ ok: false, error: 'invalid_deal_id' }, 400);
    ...
    const deal = await upsertDeal(db, parsed.value || {}, id);
    return jsonResponse({ ok: true, storage: 'd1', deal });
}
