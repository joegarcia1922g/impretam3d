...
import { listDeals, upsertDeal } from '../../../_shared/crm.js';
...
export async function onRequest(context) {
    ...
    if (request.method === 'GET') {
        const deals = await listDeals(db, request);
        return jsonResponse({ ok: true, storage: 'd1', deals });
    }

    if (request.method === 'POST') {
        const parsed = await readJsonBody(request);
        if (parsed.error) return jsonResponse({ ok: false, error: parsed.error }, parsed.error === 'payload_too_large' ? 413 : 400);
        const deal = await upsertDeal(db, parsed.value || {});
        return jsonResponse({ ok: true, storage: 'd1', deal }, 201);
    }
