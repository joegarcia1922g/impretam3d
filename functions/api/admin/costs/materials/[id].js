...
export async function onRequest(context) {
    ...
    if (!id) {
        return jsonResponse({ ok: false, error: 'invalid_material_id' }, 400);
    }
    ...
    try {
        const material = await upsertMaterial(db, parsed.value || {}, id);
        const catalog = await getCostCatalog(db);
        return jsonResponse({ ok: true, storage: 'd1', material, catalog });
    } catch (error) {
        return jsonResponse({ ok: false, error: error.message || 'material_update_failed' }, 400);
    }
}
