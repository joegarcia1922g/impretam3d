...
export async function onRequest(context) {
    ...
    try {
        const model = await upsertModel(db, parsed.value || {});
        const catalog = await getCostCatalog(db);
        return jsonResponse({ ok: true, storage: 'd1', model, catalog }, 201);
    } catch (error) {
        return jsonResponse({ ok: false, error: error.message || 'model_save_failed' }, 400);
    }
}
