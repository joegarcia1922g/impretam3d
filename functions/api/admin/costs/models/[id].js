...
import { getCostCatalog, upsertModel } from '../../../../_shared/costs.js';
...
export async function onRequest(context) {
    ...
    if (!id) {
        return jsonResponse({ ok: false, error: 'invalid_model_id' }, 400);
    }
    ...
    try {
        const model = await upsertModel(db, parsed.value || {}, id);
        const catalog = await getCostCatalog(db);
        return jsonResponse({ ok: true, storage: 'd1', model, catalog });
    } catch (error) {
        return jsonResponse({ ok: false, error: error.message || 'model_update_failed' }, 400);
    }
}
